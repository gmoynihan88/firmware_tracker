...
async def test_api_scrapers(client):
    ...
    assert "xlnaudio" in data["scrapers"]
    assert "audient" in data["scrapers"]
    assert "mooer" in data["scrapers"]
    assert "asm" in data["scrapers"]
    assert "1010music" in data["scrapers"]
    assert "oeksound" in data["scrapers"]
    assert "dirtywave" in data["scrapers"]
    assert "ssl" in data["scrapers"]
    assert "squarp" in data["scrapers"]
    ...
    assert "sequentix" in data["scrapers"]
    assert "allenheath" in data["scrapers"]
    assert "casio" in data["scrapers"]
    assert "dreadbox" in data["scrapers"]
    assert "waldorf" in data["scrapers"]
    assert "digico" in data["scrapers"]
    assert "rolandproav" in data["scrapers"]
    assert "yamahaproaudio" in data["scrapers"]
    assert "soundcraft" in data["scrapers"]
    # VST plugin scrapers
