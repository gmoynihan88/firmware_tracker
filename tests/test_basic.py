...
# --- scheduler -------------------------------------------------------------
# This is the code that runs unattended every 24 hours. Nobody is watching when it
# breaks, so the behaviour that matters is not "does it scrape" but "does it survive
# the scrape going wrong".
...
# --- arturia ---------------------------------------------------------------
# Everything here comes from two JSON endpoints, so the fixtures are the records
# themselves. Each test is a trap the API's shape sets rather than a parsing edge.
...
# --- universal audio: uafx and ox ------------------------------------------
# UA is two vendors in one file. The pedals publish dated release notes; the UADX
# plugins publish no version anywhere. Treating the whole vendor as unpublishable
# is what kept 22 dated products out of the catalogue.
...
# --- novation ---------------------------------------------------------------
# The downloads site is a convincing dead end: one shared USB driver on every
# product page, DAW scripts, and "Go to Components Web". The Components app's own
# manifest answers without a key and carries the whole catalogue.
...
# --- teenage engineering ----------------------------------------------------
# Version and date are paired by adjacency because the markup differs between
# products: OP-XY puts both in one div, EP-133 puts them in separate elements.
...
# --- akai -------------------------------------------------------------------
# Gatsby page data, where the version is only half in the version field and the
# descriptions carry file sizes that look exactly like versions.
...
# --- empress effects --------------------------------------------------------
# The versions are split across two pages: the shipping release on one with no
# date, every earlier one on the archive with a date. Reading either alone is
...
# --- fender -----------------------------------------------------------------
# fender.com is unreachable: aiohttp gets nothing, a browser gets Cloudflare. The
# Zendesk API answers with no challenge, and its articles put three near-misses
# within a few lines of the real firmware version.
