...
This scrapes 32 manufacturers on a schedule, compares what it finds against the gear
...
- **Scrapes 31 manufacturers** — Ableton, Akai, Arturia, Boss, Elektron, Eventide,
  Empress, Focusrite, iConnectivity, iZotope, Korg, Moog, Native Instruments,
  Novation, Steinberg, Strymon, Teenage Engineering, Universal Audio and more
...
- Conditional requests (`If-None-Match` / `If-Modified-Since`), so an unchanged page
  returns `304` with no body. Five of the 32 vendors send validators; for those it
...
**Add devices** from the catalogue at `/catalog`. 786 devices across 32 vendors, so it
...
| Empress Effects | Moog |
| Eventide\* | Native Instruments |
| Focusrite | Steinberg |
| iConnectivity | TAL Software |
| Keith McMillen | Universal Audio |
| Korg |  |
| Line 6 |  |
| Novation |  |
| Peterson |  |
...
src/
  ...
  scrapers/
    ...
    plugins/           # One file per manufacturer (32 scrapers)
