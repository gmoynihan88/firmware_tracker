...
This scrapes 26 manufacturers on a schedule, compares what it finds against the gear
...
- **Scrapes 26 manufacturers** — Arturia, Boss, Elektron, Eventide, Focusrite, iZotope,
  Korg, Moog, Native Instruments, Novation, Steinberg, Strymon, Universal Audio and more
...
- Conditional requests (`If-None-Match` / `If-Modified-Since`), so an unchanged page
  returns `304` with no body. Five of the 26 vendors send validators; for those it
...
**Add devices** from the catalogue at `/catalog`. 713 devices across 26 vendors, so it
...
src/
  ...
  scrapers/
    ...
    plugins/           # One file per manufacturer (26 scrapers)
