...
1. **Compute** — EFS file system and access point, CloudWatch log group, SSM parameters
   for the secrets, task definition, ECS service and Cloud Map registration. The task
   gets **no task role**: Chromium resolves DNS independently of the app's egress
   guard, so a rebinding host could otherwise reach the credential endpoint at
   `169.254.170.2`, which no security group can filter. The execution role covers SSM
   and logs. The container runs uvicorn with `--proxy-headers`, without which the login
   throttle sees API Gateway as the only client.
2. **Edge** — VPC Link, API Gateway HTTP API, CloudFront with the managed
   CachingDisabled policy on the default behaviour and a long TTL on `/static/*`
   (assets are content-hashed), and its log bucket.
3. **Deploy** — an OIDC role trusted only by
   ...
   commit SHA first.
4. **Guardrails** — a budget alarm, and a rate limit on `/login` at the edge if the
